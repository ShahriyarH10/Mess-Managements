...
async function dbGetAll(table) {
  const orderCol = table === "meals" || table === "bazar" ? "date" : table === "rent" ? "month_key" : "created_at";
...
async function getUnreadAnnouncementCount() {
  const { data: announcements } = await sb.from("announcements").select("id, created_at")
...
function buildNav() {
  ...
  document.getElementById("mobile-nav").innerHTML = `
    ${mainItems.map(i => `<button class="mob-nav-btn" onclick="navigate('${i.page}')" data-page="${i.page}">${i.icon}<span>${i.label}</span></button>`).join("")}
...
async function renderDashboard(el) {
  ...
  mM.forEach(r => Object.values(r.meals||{}).forEach(v => totalMeals+=Number(v)));
...
async function loadProfiles() {
  ...
  if (selectedProfileId) showProfileDetail(selectedProfileId,allMeals,allBazar,allRent,allUtil||[]);
}
...
async function selectProfile(id) {
  const [allM,allB,allR,{data:allU}]=await Promise.all([dbGetAll("meals"),dbGetAll("bazar"),dbGetAll("rent"),sb.from("utility_payments").select("*").eq("mess_id",messId())]);
...
function showProfileDetail(id,allM,allB,allR,allU) {
  ...
  const mealNet=s.mealBalance, rentNet=round2(s.rentPaid-s.rentDue), utilNet=round2(s.utilityPaid-s.utilityDue);
  const allMealsTotal=allM.reduce((s,r)=>s+Object.values(r.meals||{}).reduce((a,v)=>a+Number(v),0),0);
  ...
  document.getElementById("modal-content").innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:18px;padding-bottom:16px;border-bottom:1px solid var(--border);flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:14px">
        ...
        <div>
          ...
          <div style="font-size:12px;color:var(--text2);margin-top:3px">Room ${member.room||"—"} · @${member.username||"—"}</div>
        </div>
      </div>
    ...
    <div class="detail-section"><div class="detail-section-title">Share in mess (all time)</div>
    ...
    <div class="detail-section"><div class="detail-section-title">Recent months</div>
      <div class="tbl-wrap"><table><thead><tr><th>Month</th><th>Meals</th><th>Bazar</th><th>Rent paid</th><th>Utility paid</th></tr></thead>
      <tbody>${allMK.slice(-6).reverse().map(k=>{const d=s.byMonth[k]||{};return`<tr><td>${MONTHS[parseInt(k.slice(5))-1]} ${k.slice(0,4)}</td><td>${d.meals||0}</td><td>${fmtTk(d.bazar||0)}</td><td style="color:var(--green)">${fmtTk(d.rentPaid||0)}</td><td style="color:var(--green)">${fmtTk(d.utilityPaid||0)}</td></tr>`;}).join("")}</tbody></table></div>
    </div>
...
async function renderMyDashboard(el) {
  ...
  // POSTPAID bills from PREVIOUS month
  const prevBills=prevUtilRec?.bills||{};
  ...
  const todayRec=allMeals.find(r=>r.date===todayStr);
  let todayDayTotal=0, todayNightTotal=0;
  if(todayRec){ members.forEach(m=>{ todayDayTotal+=Number(todayRec.meals[m.name+"_day"]??todayRec.meals[m.name]??0); todayNightTotal+=Number(todayRec.meals[m.name+"_night"]??0); }); todayDayTotal=round2(todayDayTotal); todayNightTotal=round2(todayNightTotal); }
  ...
  el.innerHTML=`
  <div class="topbar"><div><div class="page-title">My Dashboard</div><div class="page-sub">${MONTHS[month]} ${year}</div></div></div>
  <div class="content">
    ...
    <div class="card" style="margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:16px;flex-wrap:wrap">
          <div style="text-align:right"><div style="font-size:10px;color:var(--text3)">Meal pay/get</div><div style="font-size:16px;font-weight:700;color:${mealNet>=0?"var(--green)":"var(--red)"}">${mealNet>=0?"Get ":"Pay "}${fmtTk(Math.abs(mealNet))}</div></div>
          <div style="text-align:right;padding-left:16px;border-left:1px solid var(--border)"><div style="font-size:10px;color:var(--text3)">Rent balance</div><div style="font-size:16px;font-weight:700;color:${rentNet>=0?"var(--green)":"var(--red)"}">${rentNet>=0?"+":""}${fmtTk(rentNet)}</div></div>
          <div style="text-align:right;padding-left:16px;border-left:1px solid var(--border)"><div style="font-size:10px;color:var(--text3)">Utility balance</div><div style="font-size:16px;font-weight:700;color:${utilNet>=0?"var(--green)":"var(--red)"}">${utilNet>=0?"+":""}${fmtTk(utilNet)}</div></div>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div class="stat-card" style="padding:10px"><div class="stat-label">My meals</div><div class="stat-value">${round2(myMeals)}</div><div class="stat-sub">${MONTHS[prevMonth].slice(0,3)}</div></div>
          <div class="stat-card" style="padding:10px"><div class="stat-label">Meal rate</div><div class="stat-value" style="font-size:16px">${fmtTk(mealRate)}</div><div class="stat-sub">per meal</div></div>
          <div class="stat-card" style="padding:10px"><div class="stat-label">My bazar</div><div class="stat-value" style="font-size:16px">${fmtTk(myBazar)}</div></div>
          <div class="stat-card" style="padding:10px"><div class="stat-label">Meal cost</div><div class="stat-value" style="font-size:16px">${fmtTk(mealCost)}</div></div>
        </div>
      </div>
      ${todayRec?`
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px;margin-bottom:10px">
          <div class="stat-card badge-blue" style="padding:8px;text-align:center"><div class="stat-label" style="color:var(--blue);font-size:10px">Day</div><div style="font-size:18px;font-weight:700;color:var(--blue)">${todayDayTotal}</div></div>
          <div class="stat-card badge-amber" style="padding:8px;text-align:center"><div class="stat-label" style="color:var(--amber);font-size:10px">Night</div><div style="font-size:18px;font-weight:700;color:var(--amber)">${todayNightTotal}</div></div>
          <div class="stat-card" style="padding:8px;text-align:center"><div class="stat-label" style="font-size:10px">Total</div><div style="font-size:18px;font-weight:700">${round2(todayDayTotal+todayNightTotal)}</div></div>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${members.map(m=>{ const d=Number(todayRec.meals[m.name+"_day"]??todayRec.meals[m.name]??0); const n=Number(todayRec.meals[m.name+"_night"]??0); const t=round2(d+n)||Number(todayRec.meals[m.name]||0); return`<div style="display:flex;align-items:center;gap:5px;background:var(--bg3);padding:5px 9px;border-radius:var(--radius-sm);border:1px solid var(--border)"><span style="font-size:12px;font-weight:500;color:${t>0?"var(--text)":"var(--text3)"}">${m.name}</span>${d>0?`<span class="badge badge-blue" style="font-size:9px">D${d}</span>`:""}${n>0?`<span class="badge badge-amber" style="font-size:9px">N${n}</span>`:""}${t===0?`<span class="badge badge-red" style="font-size:9px">—</span>`:""}</div>`; }).join("")}
        </div>
      </div>`:
      `<div class="card" style="display:flex;align-items:center;justify-content:center;flex-direction:column;gap:8px;min-height:140px"><div style="font-size:28px">🍽️</div><div style="color:var(--text3);font-size:13px">No meal entry today</div></div>`}
    </div>

    <div class="card" style="margin-bottom:14px">
      <div class="card-title">What I owe — ${MONTHS[month]} ${year}</div>
      <div style="font-size:11px;color:var(--red);text-transform:uppercase;letter-spacing:.6px;margin-bottom:8px;font-weight:600">🔴 Postpaid — from ${MONTHS[prevMonth]} ${prevYear} (settle now)</div>
      <div class="my-stat-row"><span class="my-stat-key">🍽️ Meal cost <span style="font-size:10px;color:var(--text3)">${round2(myMeals)} meals × ${fmtTk(mealRate)}</span></span><span class="my-stat-val">${fmtTk(mealCost)}</span></div>
      <div class="my-stat-row"><span class="my-stat-key">👩 Khala <span style="font-size:10px;color:var(--text3)">my share</span></span><span class="my-stat-val">${fmtTk(khalaShare)}</span></div>
      <div class="my-stat-row"><span class="my-stat-key">📦 Other <span style="font-size:10px;color:var(--text3)">my share</span></span><span class="my-stat-val">${fmtTk(otherShare)}</span></div>
      <div style="display:flex;justify-content:space-between;padding:7px 0;font-size:13px;color:var(--text3)"><span>Postpaid subtotal</span><span>${fmtTk(round2(mealCost+khalaShare+otherShare))}</span></div>
      <div style="font-size:11px;color:var(--blue);text-transform:uppercase;letter-spacing:.6px;margin:12px 0 8px;font-weight:600;border-top:1px solid var(--border);padding-top:12px">🔵 Prepaid — for ${MONTHS[month]} ${year} (pay upfront)</div>
      <div class="my-stat-row"><span class="my-stat-key">🏠 Room rent</span><span class="my-stat-val" style="color:${myRe.status==="paid"?"var(--green)":"var(--text)"}">${fmtTk(myRentDue)} ${myRe.status==="paid"?"✓":""}</span></div>
      <div class="my-stat-row"><span class="my-stat-key">⚡ Utility bills <span style="font-size:10px;color:var(--text3)">elec+wifi+gas share</span></span><span class="my-stat-val" style="color:${myUtilPay.status==="paid"?"var(--green)":"var(--text)"}">${fmtTk(prepaidShare)} ${myUtilPay.status==="paid"?"✓":""}</span></div>
      <div style="display:flex;justify-content:space-between;padding:7px 0;font-size:13px;color:var(--text3)"><span>Prepaid subtotal</span><span>${fmtTk(round2(myRentDue+prepaidShare))}</span></div>
      <div style="border-top:2px solid var(--border2);margin-top:4px;padding-top:12px">
        <div class="my-stat-row" style="font-size:15px"><span style="font-weight:600">Total this month</span><span style="font-weight:700">${fmtTk(round2(mealCost+khalaShare+otherShare+myRentDue+prepaidShare))}</span></div>
        <div class="my-stat-row"><span class="my-stat-key" style="color:var(--green)">− Bazar credit <span style="font-size:10px;color:var(--text3)">${MONTHS[prevMonth].slice(0,3)}</span></span><span class="my-stat-val" style="color:var(--green)">${fmtTk(myBazar)}</span></div>
        ${myUtilActualPaid>0?`<div class="my-stat-row"><span class="my-stat-key" style="color:var(--green)">− Utility already paid</span><span class="my-stat-val" style="color:var(--green)">${fmtTk(myUtilActualPaid)}</span></div>`:""}
  ...
  <div class="topbar">
    ...
    <div class="topbar-actions">
      <select class="input" id="my-prof-period" onchange="refreshMyProfile()" style="width:180px">
        <option value="1" selected>This month</option><option value="last">Last month</option>
        <option value="3">Last 3 months</option><option value="6">Last 6 months</option><option value="all">All time</option>
      </select>
    </div>
  </div>
...
async function loadMyProfile(member) {
  ...
  content.innerHTML=`<div class="card">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:18px;padding-bottom:16px;border-bottom:1px solid var(--border);flex-wrap:wrap">
      ...
      <div style="display:flex;gap:20px;flex-wrap:wrap">
        <div style="text-align:right"><div style="font-size:11px;color:var(--text3)">Meal pay/get</div><div style="font-size:18px;font-weight:700;color:${mealNet>=0?"var(--green)":"var(--red)"}">${mealNet>=0?"Get ":"Pay "}${fmtTk(Math.abs(mealNet))}</div></div>
        <div style="text-align:right;padding-left:20px;border-left:1px solid var(--border)"><div style="font-size:11px;color:var(--text3)">Rent balance</div><div style="font-size:18px;font-weight:700;color:${rentNet>=0?"var(--green)":"var(--red)"}">${rentNet>=0?"+":""}${fmtTk(rentNet)}</div></div>
        <div style="text-align:right;padding-left:20px;border-left:1px solid var(--border)"><div style="font-size:11px;color:var(--text3)">Utility balance</div><div style="font-size:18px;font-weight:700;color:${utilNet>=0?"var(--green)":"var(--red)"}">${utilNet>=0?"+":""}${fmtTk(utilNet)}</div></div>
      </div>
    </div>
...
async function renderMyPayments(el) {
  ...
  <div class="content">
    ...
    <div class="grid-2" style="gap:12px">
      <div class="card"><div class="card-title">Rent history</div>
        ${allRent.length?`<div class="tbl-wrap"><table><thead><tr><th>Month</th><th>Due</th><th>Paid</th><th>Status</th></tr></thead><tbody>${[...allRent].sort((a,b)=>b.month_key.localeCompare(a.month_key)).map(r=>{ const e=r.entries?.find(x=>x.name===member.name)||{}; if(!e.rent&&!e.paid) return""; const cls=e.status==="paid"?"badge-green":e.status==="partial"?"badge-amber":"badge-red"; return`<tr><td>${r.month_name} ${r.year}</td><td>${fmtTk(e.rent||0)}</td><td style="color:var(--green)">${fmtTk(e.paid||0)}</td><td><span class="badge ${cls}">${e.status==="paid"?"Paid":e.status==="partial"?"Part":"Due"}</span></td></tr>`; }).join("")}</tbody></table></div>`:'<div class="empty">No rent records</div>'}
      </div>
      <div class="card"><div class="card-title">Utility history</div>
        ${allUtil?.length?`<div class="tbl-wrap"><table><thead><tr><th>Month</th><th>My share</th><th>Paid</th><th>Status</th></tr></thead><tbody>${allUtil.map(r=>{ const bills=r.bills||{}; const total=["elec","wifi","gas","khala","other"].reduce((s,k)=>s+(bills[k]||0),0); const perHead=members.length>0?round2(total/members.length):0; const p=(r.payments||{})[member.name]||{}; const cls=p.status==="paid"?"badge-green":p.status==="partial"?"badge-amber":"badge-red"; return`<tr><td>${r.month_name} ${r.year}</td><td>${fmtTk(perHead)}</td><td style="color:var(--green)">${fmtTk(p.paid||0)}</td><td><span class="badge ${cls}">${p.status==="paid"?"Paid":p.status==="partial"?"Part":"Due"}</span></td></tr>`; }).join("")}</tbody></table></div>`:'<div class="empty">No utility records</div>'}
      </div>
    </div>
  </div>`;
}
...
async function renderMessOverview(el) {
  ...
  mM.forEach(r=>Object.values(r.meals||{}).forEach(v=>totalM+=Number(v)));
...
function closeModal() { document.getElementById("modal-bg").classList.remove("open"); const m=document.querySelector(".modal"); if(m) m.classList.remove("modal-wide"); }
let toastTimer;
function toast(msg,type) { const el=document.getElementById("toast"); el.textContent=msg; el.className="show"+(type?" "+type:""); clearTimeout(toastTimer); toastTimer=setTimeout(()=>el.className="",3200); }
...
async function init() {
  ...
  if(currentUser) {
    ...
    if(currentUser.memberId){
      try {
        const {data:fresh}=await sb.from("members").select("role,name,username").eq("id",currentUser.memberId).maybeSingle();
        if(fresh){ currentUser.role=fresh.role; saveSession(currentUser,currentMess); }
        else { clearSession(); showLanding(); return; }
